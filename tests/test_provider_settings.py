...
def test_provider_settings_ui_fields_override_runtime_env(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("CODE_RESEARCH_AGENT_SECRET_STORE_PATH", str(tmp_path / "secrets.json"))
    ...
    assert llm.max_output_tokens == 900
    assert vision_settings.timeout_seconds == 13
    assert vision_settings.max_retries == 4
    assert vision_settings.max_output_tokens == 901
    assert vision_settings.qwen_vl.supports_json_object is True
    assert vision_settings.qwen_vl.disable_thinking is True
    assert image_settings.timeout_seconds == 14
    assert image_settings.max_retries == 2
...
def test_provider_settings_write_security_checks_remote_origin_and_admin_token(monkeypatch, tmp_path: Path):
    ...
    with TestClient(app, client=("203.0.113.10", 50000)) as remote_client:
        remote_blocked = remote_client.put("/settings/providers/deepseek", json=payload)
    ...
    with TestClient(app, client=("203.0.113.10", 50000)) as remote_client:
        missing_token = remote_client.put("/settings/providers/deepseek", json=payload, headers={"origin": "https://admin.example"})
