...
test("shows evaluation runs, bad cases, and pending alignment benchmark", async () => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
    ...
    const body = url.includes("/evaluation/runs") ? { items: [{ run_id: "run-1", dataset_version_id: "dataset-v1", subject_id: "subject-1", mode: "deterministic_fixture", status: "completed", complete: true, incomplete_reason_codes: [], case_counts: { total: 6, completed: 6 }, created_at: new Date().toISOString() }] }
      ...
      : url.includes("/evaluation/baselines") ? { items: [] }
      : url.includes("/evaluation/comparisons") ? { items: [] }
