...
v1.3.4 是 Provider 设置稳定性修补版：Origin 校验改为解析后精确匹配 allowlist；DeepSeek/Qwen、Qwen-VL/GLM、Qwen-Image/Seedream 使用各自的超时、重试和输出上限；环境变量 API Key 在 UI 中只读。v1.3.4 仍只支持同步生图，不包含视频功能，也不进入 v1.4 范围。
