...
v1.3.0 在 v1.2.4 的基础上新增本地优先的教学图生成：规则事实和 `diagrams.json` 先生成 `TeachingDiagramSkeleton`，本地 BlueprintRenderer 输出 SVG/PNG；AI 图片生成和 VLM 审查拥有独立授权、预算、缓存和回退策略。当前仍不包含视频、登录、复杂部署、PDF 导出或复杂 RAG。
