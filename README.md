...
v1.3.5 是全仓审计与轻量化版：统一 Provider 配置解析，隔离可序列化分析选项与进程内 Runtime，建立 Prompt Registry，将 Mermaid 改为按需加载，并清理已确认不可达的代码。异步图片内部实现已移除；`supports_async` 仅保留一版弃用兼容，`false` 被忽略，`true` 返回 422。
