...
v1.3.1 在 v1.3.0 的基础上把教学图流程修成真实闭环：AI raw 只作为视觉背景层，`TeachingDiagramCompositor` 会确定性覆盖模块文字、箭头、Shape、公式、图例和步骤；VLM Review 通过现有 `VisionModelRouter` 调用 Qwen-VL/GLM-4.5V，失败时最多触发一次 Seedream 重生成，仍不通过则回退本地 Blueprint。当前仍不包含视频、登录、复杂部署、PDF 导出或复杂 RAG。
