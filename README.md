...
v1.3.2 是教学图稳定版：Review cache 会在命中后重新做 Schema、diagram_id 和严格质量策略校验；Qwen-Image/Seedream 支持独立请求尺寸和可配置结果域名白名单；PNG 箭头按最后一段方向绘制；TeachingDiagramPlanNode 可在独立预算下调用 DeepSeek/Qwen 生成教学文案，但结构仍完全来自规则 Skeleton。当前仍不包含视频、登录、复杂部署、PDF 导出或复杂 RAG。
