...
v1.3.3 是 Provider 设置与教学图体验修补版：Provider 设置中心保存的超时、重试、输出长度和视觉/图片开关会覆盖环境变量；UI 只暴露安全的 Key 来源，不返回真实 Key；v1.3.3 仍只支持同步生图，`supports_async=true` 会被拒绝。当前仍不包含视频、登录、复杂部署、PDF 导出或复杂 RAG。
