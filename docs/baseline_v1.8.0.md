...
| v1.8 Implementation Commit | `0bbba875082e9964bae3b7b9e4056c077ec8b1ad` (`feat: implement unified observability v1.8.0`) |
| Release Identity | annotated tag `v1.8.0`，指向包含本基线冻结记录的 release commit |
| 工作区 | v1.8 实现与基线冻结提交后为干净工作区 |
| Git 事实结论 | v1.8 实现已形成独立 commit；`v1.8.0` tag 是后续 Evaluation provenance 的正式版本入口 |
...
v1.9 Evaluation Run 必须记录实际 code commit SHA；使用正式发布基线时还应记录 `v1.8.0` tag。若未来在该 tag 之后存在未提交 patch，仍必须额外记录 worktree patch/content hash，不能只记录 tag。
...
1. `ALIGNMENT_BENCHMARK_PENDING` 未关闭，是 v1.9-a 的优先任务。
2. Retrieval/Agent benchmark 是 synthetic fixture，缺少真实多仓库分布和冻结 outcome。
3. Answer 没有独立语义 Gold；LLM Judge 不能替代人工/确定性 Gold。
4. OTel、Qdrant/FastEmbed 与真实 Provider 路径未在当前环境验收。
5. Observability API 依赖保守本地管理员策略，不是完整认证/RBAC。
6. 前端 build 有大 chunk warning；不影响本次 build 通过，但应在后续单独优化。
