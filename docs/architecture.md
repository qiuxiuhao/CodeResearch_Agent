...
- Evaluation 评测层：`backend/app/evaluation/` 使用不可变 Subject/Dataset/Run、六组件 Adapter、版本化 Metric/Gate、Bad Case/Promotion 与受控 Replay；它只读业务 Artifact，不进入业务 Graph。
...
10. Evaluation Coordinator在独立Trace/Lease中运行deterministic或受控Live Case；结果写入独立Evaluation Store，不回写Gold、业务Run或Checkpoint。
